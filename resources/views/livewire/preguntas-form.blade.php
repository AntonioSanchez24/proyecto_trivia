<div>
    <div class="p-6 border border-black border-solid border-2 rounded-lg shadow-lg bg-white">
        ...
        <form wire:submit.prevent="manejarDatos" method="POST">
    ...
    <div class="p-6 border border-black border-solid border-2 rounded-lg shadow-lg bg-white">
        ...
        <div class="max-w-7xl mx-auto py-10 sm:px-6 lg:px-8">
            @foreach ($preguntasArray as $preguntaIndex => $pregunta)
                <div wire:key='pregunta{{ $preguntaIndex }}' class="border p-4 mb-4 rounded-md shadow-sm bg-gray-50">
                    <h1 class="text-2xl mb-2 font-semibold text-gray-700">Pregunta: {{ $pregunta['pregunta'] }}</h1>
                    ...
                    <button wire:click="cargarPregunta({{ $pregunta['id'] }})"
                        class="ml-2 px-4 py-2 bg-green-500 text-white rounded-md hover:bg-green-400">Editar</button>
                </div>
            @endforeach
        </div>
    </div>
